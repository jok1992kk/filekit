...
export const metadata: Metadata = {
  ...
  description:
    "Token packs for preparing product photos — buy any time, larger packs cost less per token.",
};

export default async function TokensPage() {
  const user = await getCurrentUser();
  const balance = user ? tokenBalance(user) : null;
        ...
        <div className="max-w-[640px]">
          ...
          {balance !== null ? (
            <p className="mt-3 text-[16px] leading-[1.55]">
              You have <b className="font-medium text-ink">{formatCount(balance)} tokens</b>{" "}
              available. Top up any time — larger packs cost less per token.
            </p>
          ) : (
            <p className="mt-3 text-[16px] leading-[1.55]">
              {freeTokensLine} Top up any time — larger packs cost less per token.{" "}
