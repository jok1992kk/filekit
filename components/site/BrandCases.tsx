  ...
  return (
    <Section id="brand-cases" className="pt-0 max-mob:pt-0">
      <Container>
        <SectionHead
          eyebrow="Demo brands"
          title="Made-up brands. Real workflows."
          lead="Purpose-built examples show what each utility actually does — no generic stock grid and no invented customer claims."
        />
          ...
          {cases.map((item) => (
            <article key={item.brand} className="group overflow-hidden rounded-[14px] border border-border bg-white">
              <div className={`relative aspect-[4/3] overflow-hidden ${item.tone}`}>
                ...
                <span className="absolute left-3 top-3 rounded-full border border-white/70 bg-white/90 px-2.5 py-1 text-[10px] font-medium text-ink shadow-sm backdrop-blur">Concept brand</span>
              </div>
              <div className="p-5">
